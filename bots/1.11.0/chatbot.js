...
export default class Chatbot {
  ...
  async handleFileUpload(file) {
    // Validate session and org IDs are available
    if (!this.sessionId) {
      alert('Please send a message first to start a session');
      return;
    }

    if (!this.orgId || !this.orgGroupId) {
      alert('Session not properly initialized. Please refresh and try again.');
      return;
    }
    ...
    try {
      ...
      // Send notification message that file was uploaded
  ...
  async sendUserMessage(message) {
    ...
    try {
      ...
      if (!this.sessionId) {
        ...
        this.fileUploadEnabled = data.file_upload_enabled || false;
        ...
        if (data.file_upload_enabled !== undefined) this.fileUploadEnabled = data.file_upload_enabled;
      }
